...
class LeaveTraceController extends GetxController {
    ...
    try {
      ...
      for (var provinceDoc in provincesSnapshot.docs) {
        ...
        if (distanceInMeters <= 4000) {
          ...
          username = provinceDoc['username'];
          profileImage = provinceDoc['profileImage'];
          await sendNotifiction(
              receiverId, user!.displayName, ' some one add a finger print');
        }
      }
    } catch (xerroe) {
  ...
  Future sendNotifiction(
      String receiverId, String? nameuser, String action) async {
